...
-- ============================================================
-- CONFIG SYSTEM  (ported from deobfuscated ArchConfig source)
-- ============================================================
local ConfigTab   = Library:Tab("Config", 98159911363596)
local ConfigLeft  = ConfigTab:Group("Config Manager")
local ConfigRight = ConfigTab:Group("Options")

local CONFIG_FOLDER  = "ArchScripts"
local currentCfgName = "default"  -- tracks what's currently selected
...
-- Color3 → {R, G, B}  (deobf source stores colors this way)
local function c3ToArr(c)
    return { c.R, c.G, c.B }
end

-- {R, G, B} or {r, g, b} → Color3
local function arrToC3(t)
    if type(t) == "table" and (#t == 3 or (t.r and t.g and t.b)) then
        return Color3.new(t[1] or t.r, t[2] or t.g, t[3] or t.b)
    end
    return nil
end

-- Serialise S for JSON.  Color3 saved as [R,G,B].  EnumItem as Name string.
local function serialiseS()
    local out = {}
    for k, v in pairs(S) do
        ...
        elseif t == "EnumItem" then
            out[k] = v.Name
        elseif t ~= "function" and t ~= "RBXScriptConnection"
               and t ~= "Instance" and t ~= "userdata" then
            out[k] = v
        end
    end
    return out
...
-- Write serialised S into the live S table, restoring Color3 / EnumItem
local function deserialiseIntoS(raw)
    for k, v in pairs(raw) do
        if type(v) == "table" then
            local asColor = arrToC3(v)
            S[k] = asColor or v
        elseif type(v) == "string" then
            -- restore keybind EnumItems
            if k == "triggerbotKey" then
                local ok, ev = pcall(function() return Enum.KeyCode[v] end)
                S[k] = (ok and ev) or v
            else
                S[k] = v
            end
...
local function saveConfig(name)
    ...
    if ok then
        currentCfgName = name
        Library:Notify("Config \"" .. name .. "\" saved!", "success")
    else
        Library:Notify("Save failed: " .. tostring(err), "warning")
    end
    return ok
end
...
local function loadConfig(name, silent)
    ...
    if not isfile(path) then
        if not silent then
            Library:Notify("Config \"" .. name .. "\" not found!", "warning")
        end
        return false
    end
    ...
    if not ok or not result then
        if not silent then
            Library:Notify("Parse error in \"" .. name .. "\"", "warning")
        end
        return false
    end

    deserialiseIntoS(result)
    ApplyAllSettings()
...
local function deleteConfig(name)
    if not name or name == "" or name == "default" then
        Library:Notify("Cannot delete the default config!", "warning")
        return false
    end
    local path = CONFIG_FOLDER .. "/" .. name .. ".json"
    if not isfile(path) then
    ...
    if ok then
        ...
        Library:Notify("Delete failed: " .. tostring(err), "warning")
    end
    return ok
end
...
-- Config name input (type the name you want to save/load/delete)
ConfigLeft:Textbox({
    Name    = "Config Name",
...
ConfigLeft:Button({
    Name     = "Load Config",
    Callback = function()
        loadConfig(currentCfgName, false)
    end
})

-- Delete  (two-click confirmation)
do
    local _lastDeleteClick = 0
    ConfigLeft:Button({
        Name     = "Delete Config",
        Callback = function()
            local now = tick()
            if now - _lastDeleteClick < 0.7 then
                deleteConfig(currentCfgName)
                _lastDeleteClick = 0
            else
                _lastDeleteClick = now
                Library:Notify("Click again to confirm delete", "warning")
            end
        end
    })
end

-- Refresh / list available configs
ConfigLeft:Button({
    Name     = "List Configs",
    Callback = function()
        local names = listConfigs()
        local display = table.concat(names, ", ")
        Library:Notify("Configs: " .. display, "success")
    end
})
...
ConfigRight:Toggle({
    ...
    Default = false,
    Tooltip = "Loads the 'default' config automatically on startup",
...
local function applyPreset(data, name)
    ...
    Library:Notify(name .. " preset applied", "success")
end
...
ConfigRight:Button({
    Name     = "Preset: Legit",
    Variant  = "Primary",
    Callback = function() applyPreset(legitPreset, "Legit") end
})

ConfigRight:Button({
    Name     = "Preset: Rage",
    Variant  = "Danger",
    Callback = function() applyPreset(ragePreset, "Rage") end
})
