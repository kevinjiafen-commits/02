...
-- ============================================================
-- Imported Ragebot + Flickbot (merged from arch rage (1).txt)
-- Namespaced to avoid collisions with the original Arch script.
-- ============================================================
-- ===== 全局设置 (UI 绑定) =====
...
-- 解析本地 Fighter
local RageFighterController = nil
local function ResolveFighterController()
    if RageFighterController then return RageFighterController end
...
local RagebotGroup = Rage:Group("Ragebot")
RagebotGroup:Toggle({Name = "Enabled", Tooltip = "启用 Ragebot", Callback = function(v) RageSettings.Enabled = v end})
RagebotGroup:Slider({Name = "Stability", Min = 0, Max = 1.5, Default = 0.15, Callback = function(v) RageSettings.Stability = v end})
RagebotGroup:Slider({Name = "Shoot Frames", Min = 1, Max = 5, Default = 1, Unit = "帧", Callback = function(v) RageSettings.ShootFrames = v end})
RagebotGroup:Toggle({Name = "Prioritize Hackers", Callback = function(v) RageSettings.PrioritizeHackers = v end})
RagebotGroup:Toggle({Name = "Use Primary", Default = true, Callback = function(v) RageSettings.WeaponPrimary = v end})
RagebotGroup:Toggle({Name = "Use Secondary", Default = true, Callback = function(v) RageSettings.WeaponSecondary = v end})
RagebotGroup:Toggle({Name = "Use Melee", Default = true, Callback = function(v) RageSettings.WeaponMelee = v end})
RagebotGroup:Dropdown({Name = "On Empty", Options = {"Swap","Reload","SwapOrReload"}, Default = "SwapOrReload", Callback = function(v) RageSettings.OnEmpty = v end})
RagebotGroup:Dropdown({Name = "Evasion Mode", Options = {"Random","Translocate","ProjectileBreaker"}, Default = "Random", Callback = function(v) RageSettings.EvasionMode = v end})
RagebotGroup:Slider({Name = "Translocate Offset", Min = -5, Max = 5, Default = -5, Callback = function(v) RageSettings.TranslocateOffset = v end})
RagebotGroup:Slider({Name = "Random Base Radius", Min = 5, Max = 100000000, Default = 100, Callback = function(v) RageSettings.RandomBaseRadius = v end})
RagebotGroup:Slider({Name = "Random Range Factor", Min = 0, Max = 1, Default = 0.5, Callback = function(v) RageSettings.RandomRadiusFactor = v end})
RagebotGroup:Toggle({Name = "Character Origin", Callback = function(v) RageSettings.RandomAnchorFromCharacter = v end})

local FlickbotGroup = Rage:Group("Flickbot")
FlickbotGroup:Toggle({Name = "Enabled", Callback = function(v) RageSettings.FlickbotEnabled = v end})
FlickbotGroup:Keybind({Name = "Keybind", Default = Enum.KeyCode.Unknown, Callback = function(k) RageSettings.FlickbotKeybind = k end})
FlickbotGroup:Toggle({Name = "Auto Shoot", Callback = function(v) RageSettings.FlickbotShot = v end})
FlickbotGroup:Slider({Name = "Shot Delay", Min = 0, Max = 250, Default = 0, Unit = "ms", Callback = function(v) RageSettings.FlickbotShotDelay = v end})
FlickbotGroup:Slider({Name = "Cooldown", Min = 0, Max = 2000, Default = 250, Unit = "ms", Callback = function(v) RageSettings.FlickbotCooldown = v end})
FlickbotGroup:Slider({Name = "Flick Duration", Min = 30, Max = 400, Default = 110, Unit = "ms", Callback = function(v) RageSettings.FlickbotDuration = v end})
FlickbotGroup:Slider({Name = "Curvature", Min = 0, Max = 50, Default = 12, Callback = function(v) RageSettings.FlickbotCurvature = v end})
FlickbotGroup:Slider({Name = "Humanness", Min = 0, Max = 100, Default = 30, Unit = "%", Callback = function(v) RageSettings.FlickbotHumanness = v end})
...
local function saveConfig()
    ...
    for k, v in pairs(RageSettings) do
        if typeof(v) == "EnumItem" then
            data.RageSettings[k] = {__type = "Enum", enum = tostring(v.EnumType), name = v.Name}
        elseif type(v) ~= "function" and typeof(v) ~= "RBXScriptConnection" and typeof(v) ~= "Instance" and typeof(v) ~= "userdata" then
            data.RageSettings[k] = v
        end
    end
...
local function loadConfig(silent)
    ...
    for k, v in pairs(data) do
        if k == "RageSettings" then
            if type(v) == "table" then
                for rk, rv in pairs(v) do
                    if type(rv) == "table" and rv.__type == "Enum" then
                        local enumType = Enum[rv.enum]
                        if enumType then RageSettings[rk] = enumType[rv.name] end
                    else
                        RageSettings[rk] = rv
                    end
                end
...
task.delay(1, function()
    if game.GameId == 6035872082 then
        Library:Notify("Ragebot + Flickbot 已加载", "success")
        RunService.Heartbeat:Connect(function(dt)
            pcall(UpdateRagebot, dt or 0)
            pcall(UpdateFlickbot, dt or 0)
        end)
    else
        Library:Notify("当前游戏不是 RIVALS，Ragebot 不会工作", "warning")
    end
end)
