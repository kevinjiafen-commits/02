...
local WorldTab = Library:Tab("World", 6031075938)
...
local function applyWorldLighting()
    if WorldSettings.LightingEnabled then
        Lighting.Brightness = WorldSettings.Brightness
        Lighting.Ambient = WorldSettings.AmbientColor
...
local LightingGroup = WorldTab:Group("Lighting")

LightingGroup:Toggle({
    Name = "Enable Lighting Changer",
    Default = false,
    Callback = function(v)
        WorldSettings.LightingEnabled = v
        applyWorldLighting()
    end
})

LightingGroup:Slider({
    Name = "Brightness",
    Min = 0,
    Max = 10,
    ...
    Increment = 0.1,
    Callback = function(v)
        WorldSettings.Brightness = v
        if WorldSettings.LightingEnabled then Lighting.Brightness = v end
    end
})

LightingGroup:ColorPicker({
    Name = "Ambient Color",
    Default = Lighting.Ambient,
    Callback = function(v)
        WorldSettings.AmbientColor = v
        if WorldSettings.LightingEnabled then Lighting.Ambient = v end
    end
})
...
LightingGroup:ColorPicker({
    ...
    Callback = function(v)
        WorldSettings.OutdoorAmbient = v
        if WorldSettings.LightingEnabled then Lighting.OutdoorAmbient = v end
    end
})
...
LightingGroup:Slider({
    ...
    Unit = " hrs",
    Callback = function(v)
        WorldSettings.ClockTime = v
        if WorldSettings.LightingEnabled then Lighting.ClockTime = v end
    end
})
...
LightingGroup:Slider({
    ...
    Unit = " studs",
    Callback = function(v)
        WorldSettings.FogEnd = v
        if WorldSettings.LightingEnabled then Lighting.FogEnd = v end
    end
})
...
LightingGroup:ColorPicker({
    ...
    Callback = function(v)
        WorldSettings.FogColor = v
        if WorldSettings.LightingEnabled then Lighting.FogColor = v end
    end
})

LightingGroup:Button({
    Name = "Reset Lighting",
    Callback = function()
        WorldSettings.LightingEnabled = false
        applyWorldLighting()
        Library:Notify("Lighting reset", "info")
    end
})

local EnvGroup = WorldTab:Group("Environment")

local worldWeatherPart = nil

local function clearWorldWeather()
    if worldWeatherPart then
        pcall(function() worldWeatherPart:Destroy() end)
        worldWeatherPart = nil
    end
end

local function createWorldWeather(texture, rate, speed1, speed2)
    clearWorldWeather()
    local cam = workspace.CurrentCamera
    if not cam then return end
    local part = Instance.new("Part")
    part.Name = "WorldWeatherEmitter"
    part.Anchored = true
    ...
    emitter.Rate = rate
    emitter.Speed = NumberRange.new(speed1, speed2)
    emitter.Lifetime = NumberRange.new(5, 8)
    emitter.SpreadAngle = Vector2.new(25, 25)
    emitter.Parent = part
    worldWeatherPart = part
    RunService.RenderStepped:Connect(function()
        if worldWeatherPart and workspace.CurrentCamera then
            worldWeatherPart.Position = workspace.CurrentCamera.CFrame.Position + Vector3.new(0,60,0)
        end
    end)
end
...
EnvGroup:Dropdown({
    Name = "Weather",
    Options = {"None", "Snow", "Rain", "Fog"},
    Default = "None",
    Callback = function(v)
        if v == "Snow" then
            createWorldWeather("rbxassetid://815438660", 250, 8, 14)
            Lighting.FogEnd = 250
        elseif v == "Rain" then
            createWorldWeather("rbxassetid://241594314", 900, 90, 120)
            Lighting.FogEnd = 180
        elseif v == "Fog" then
            clearWorldWeather()
            ...
            clearWorldWeather()
            if not WorldSettings.LightingEnabled then
                Lighting.FogEnd = WorldOriginals.FogEnd
            end
        end
    end
...
EnvGroup:Dropdown({
    Name = "Sky",
    Options = {"Default", "Cloudy", "Space", "Purple Nebula", "Sunset", "Blue Sky", "Dark Night"},
    Default = "Default",
    Callback = function(v)
        local ids = {
            Cloudy = "rbxassetid://159454299",
            Space = "rbxassetid://149397692",
            ["Purple Nebula"] = "rbxassetid://570557514",
            Sunset = "rbxassetid://600830446",
            ["Blue Sky"] = "rbxassetid://7018684000",
            ["Dark Night"] = "rbxassetid://6285719338"
        }
        local old = Lighting:FindFirstChildOfClass("Sky")
        if old then old:Destroy() end
        if v ~= "Default" and ids[v] then
        ...
        if v then
            for _, obj in pairs(workspace:GetDescendants()) do
                if obj:IsA("BasePart") then
                    ...
                    if model and game:GetService("Players"):GetPlayerFromCharacter(model) then continue end
                    pcall(function()
...
EnvGroup:Toggle({
    ...
    Callback = function(v)
        if v then
            ...
            Lighting.GlobalShadows = WorldOriginals.GlobalShadows
            if not WorldSettings.LightingEnabled then
                Lighting.FogEnd = WorldOriginals.FogEnd
            end
...
-- Preset configs: each button loads its own preset without touching the normal config file.
local function applyPreset(data, name)
    for k, v in pairs(data) do
        S[k] = v
    end

    -- Keep the existing runtime apply path so the loaded values take effect immediately.
    ApplyAllSettings()

    if S.undergroundActive and _G.SetUndergroundDepthExt then
        _G.SetUndergroundDepthExt(S.undergroundDepth)
    end

    Library:Notify(name .. " config loaded", "success")
end
...
local rageConfig = {
    ...
    fireDelay = 1,
    instantAdsEnabled = true,
    noEquipAnimEnabled = true,
    noShootAnimEnabled = true,
    ...
    undergroundDepth = -2,
    fistActive = true,
    irregularMoveActive = true
}

ConfigGroup:Button({
    Name = "legit",
    Callback = function()
        applyPreset(legitConfig, "Legit")
    end
})
