...
-- ============================================================
-- DeviceSpoof — Kicia [17425] full upvalue-swap port
-- Replaces the ControlsController upvalue inside
-- FighterController._ReplicateControls with a proxy table
-- that answers only CurrentControls, kicks on anything else.
-- ============================================================
local DeviceSpoof = {
    Hook = nil,
    Controller = nil,
...
local function _dsResolveControlsController()
    if DeviceSpoof.Controller then return DeviceSpoof.Controller end
    local playerScripts = LocalPlayer:FindFirstChild('PlayerScripts')
    ...
    if ok and type(ctrl) == 'table' then
        DeviceSpoof.Controller = ctrl
    end
    return DeviceSpoof.Controller
end

local function _dsReadEnabled()
    return S.deviceSpoofEnabled == true
end

local function _dsReadSpoofType()
    return type(S.spoofDevice) == 'string' and S.spoofDevice or 'VR'
end

local function _dsResolveReplicateControls()
    local fc = ResolveFighterController()
    if type(fc) ~= 'table' then return nil, nil end
    local mt  = getmetatable(fc)
    local cls = type(mt) == 'table' and rawget(mt, '__index') or nil
    local fn  = type(cls) == 'table' and rawget(cls, '_ReplicateControls') or nil
    if type(fn) ~= 'function' then return nil, fc end
    return fn, fc
end

local function _dsForceReplicate()
    local fn, fc = _dsResolveReplicateControls()
    if fn and fc then pcall(fn, fc) end
end

local function _dsAttemptLoadHook()
    if DeviceSpoof.Hook then return true end
    if type(debug.getupvalues) ~= 'function' or type(debug.setupvalue) ~= 'function' then
        return false
    end
    local cc = _dsResolveControlsController()
    if type(cc) ~= 'table' then return false end
    local fn = _dsResolveReplicateControls()
    if type(fn) ~= 'function' then return false end
    local upvalueIndex
    for idx, val in pairs(debug.getupvalues(fn)) do
        if val == cc then upvalueIndex = idx; break end
    end
    if upvalueIndex == nil then return false end
    local original = debug.getupvalue(fn, upvalueIndex)
    -- Kicia's proxy [17552]: kick guard on unexpected reads
    debug.setupvalue(fn, upvalueIndex, setmetatable({}, {
        __index = function(_, key)
            if key ~= 'CurrentControls' then
                LocalPlayer:Kick('Unexpected behavior (client misc 2)')
                return nil
            end
            local value = DeviceSpoof.ControlsMap[_dsReadSpoofType()]
            if value == nil then
    ...
    DeviceSpoof.Hook = { Getter = fn, UpvalueIndex = upvalueIndex, OriginalValue = original }
    return true
end

local function _dsAttemptUnloadHook()
    local hook = DeviceSpoof.Hook
    if not hook then return end
    DeviceSpoof.Hook = nil
    pcall(debug.setupvalue, hook.Getter, hook.UpvalueIndex, hook.OriginalValue)
end

local function applyDeviceSpoof()
    if _dsReadEnabled() then
        if _dsAttemptLoadHook() then
            _dsForceReplicate()
        end
    elseif DeviceSpoof.Hook then
        _dsAttemptUnloadHook()
        _dsForceReplicate()
    end
end

local function destroyDeviceSpoof()
    if DeviceSpoof.Hook then
        _dsAttemptUnloadHook()
        _dsForceReplicate()
    end
end
...
-- ============================================================
-- RootDesync — Kicia t201 full port (GUID-keyed bind,
-- _RenderStepUpdate pattern, _oldCFrame restore guard)
-- ============================================================
...
function RootDesync.new(rootPart)
    local self = setmetatable({
        _rootPart = rootPart,
        _boundId  = _httpSvc:GenerateGUID(false),
...
local PartGlue = {}
-- ============================================================
-- PartGlue — Kicia t116 full port: refcounted weld suspension
-- + PhysicsRepRootPart binding + VOID_CFRAME placement
-- ============================================================
...
function PartGlue:_SetupGlue(part)
    ...
    if weld ~= nil then
        ...
        if originalPart1 ~= nil then weld.Part1 = nil end
        part.Anchored = true
    end
...
function PartGlue:_ReleaseGlue(part)
    ...
    entry.refCount = entry.refCount - 1
    if 0 < entry.refCount then return end
...
function PartGlue:Acquire(ourPart, hitboxPart, useRotation)
    ...
    if bound ~= hitboxPart then
        if bound ~= nil then self:_ReleaseGlue(bound) end
        self:_SetupGlue(hitboxPart)
        self._bindings[ourPart] = hitboxPart
    end
    local cf = CFrame.new(VOID_CFRAME.Position)
    if useRotation then cf = cf * hitboxPart.CFrame.Rotation end
...
-- ============================================================
-- ViewAngleDriver — Kicia t1141 full port
-- Hooks ClientFighterCharacterJoints.Update +
-- FighterController._CameraReplicationLoop to inject a spoofed
-- CameraRotationRaw. The encode functions are exact reconstructions
-- from Kicia's deobfuscated bodies (anomalies A/B/C resolved).
-- ============================================================
local function _encodeSingle(n)
    if n == n then
        return utf8.char(math.clamp(math.floor(n % (2 * math.pi) / math.pi / 2 * 256 + 0.5), 0, 255))
    end
    return utf8.char(0)
end
local function _encodeCameraRotation(v)
    if v == v then
...
local function _decodeSingle(s)
...
local function _anglesToRaw(a)
    ...
    return Vector2.new(
        _decodeSingle(_encodeSingle(math.rad(a.pitch))),
        _decodeSingle(_encodeSingle(math.rad(a.yaw)))
    )
end
local function _anglesToEncoded(a)
    ...
    return _encodeSingle(math.rad(a.pitch)) .. _encodeSingle(math.rad(a.yaw))
end

-- Module resolver (needed by ViewAngleDriver joint hook)
local function _requireClientModule(name)
    local mod = ReplicatedStorage:FindFirstChild('Modules')
        and ReplicatedStorage.Modules:FindFirstChild(name)
    if not mod then return nil end
    local ok, result = pcall(require, mod)
    return ok and result or nil
end

local function _rawWrite(obj, key, value)
...
function ViewAngleDriver:_LoadJointsHook()
    ...
    local joints   = _requireClientModule('ClientFighterCharacterJoints')
    ...
    local function hooked(jointsSelf, a, b)
        ...
        if cf == nil then return original(jointsSelf, a, b) end
        if rawget(cf, 'IsLocalPlayer') == true then
            local winning = driver._winning
            if winning ~= nil then
                _rawWrite(b, 'CameraRotationRaw', _anglesToRaw(winning))
            end
        end
...
function ViewAngleDriver:_LoadReplicationHook()
    if self._replicationRestore ~= nil then return true end
    local fc   = ResolveFighterController()
    local mt   = fc and getmetatable(fc) or nil
    local proto = mt and rawget(mt, '__index') or nil
    local loop  = proto and rawget(proto, '_CameraReplicationLoop') or nil
    if not proto or not fc or loop == nil then return false end
    if type(debug.getupvalues) ~= 'function' or type(debug.setupvalue) ~= 'function' then
        return false
    end
    ...
    if utilIndex == nil or util == nil then return false end
    local driver      = self
    local instance    = fc
    local replacement = {}
    function replacement.EncodeCameraRotation(_, raw)
        if next(driver._slots) == nil and not driver._fullySuppressed then
            return _encodeCameraRotation(raw)
        end
        _rawWrite(instance, '_replication_stopped', false)
        local last = rawget(instance, '_last_encoded_camera_rotation')
        if not last then last = _encodeCameraRotation(raw) end
        return last
    end
    if not pcall(debug.setupvalue, loop, utilIndex, replacement) then return false end
...
function ViewAngleDriver:Flush()
    ...
    if remote then
        fireServerNative(remote, _anglesToEncoded(winning), nil)
    end
end
...
-- ============================================================
-- CharacterController — wraps RootDesync + ViewAngleDriver
-- ============================================================
